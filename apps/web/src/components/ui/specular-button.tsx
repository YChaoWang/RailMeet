...
const SIZES: Record<ButtonSize, string> = {
  ...
  lg: 'min-h-12 text-[1.15rem] px-10 py-[18px]',
};
...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    if (!btn || !fx || reduceMotion || disabled) {
      return;
    }
  ...
  const surfaceClass = cn(
    'relative m-0 inline-flex cursor-pointer items-center justify-center border-none font-sans font-medium leading-none tracking-[0.01em] outline-none transition-transform duration-150',
    'active:scale-[0.97] disabled:cursor-default disabled:opacity-55 disabled:active:scale-100',
  ...
  const surfaceStyle = {
    ...
    background:
      tintOpacity > 0
        ? `color-mix(in srgb, ${tint} ${Math.round(tintOpacity * 100)}%, transparent)`
        : baseColor,
  } as CSSProperties;
  ...
  const inner = (
    <>
      <span
        ref={fxRef}
        className="pointer-events-none absolute inset-[-20px] z-[1]"
        aria-hidden
      />
